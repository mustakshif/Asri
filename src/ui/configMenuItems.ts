...
import { asriConfigs, runtime, getI18nText } from "../core/config";
import { initAsriConfigMenuItemClick } from "./configEventHandlers";
import { asriPrstPalettes } from "../core/palette";
import {
  asriChromaSlider,
  colorPicker,
...
const paletteMenuItem = (paletteID: string) => {
  const displayName = getI18nText(paletteID);
...
export async function createBarModeMenuItems(e: Event) {
  ...
  const asriConfigMenuHTML = `
        ...
        <div class="menu-item__subtitle" style="user-select: none;">${getI18nText("palette-presets")}</div>
        ...
        <button class="b3-menu__item asri-config" id="pickColor">
            ...
            <input id="asriColorPicker" type="color" value="${asriConfigs[runtime.mode].userCustomColor}">
            <label for="asriColorPicker" class="be-menu__label">${getI18nText("pickColor")}</label>
        </button>
        <button class="b3-menu__item asri-config" id="followSysAccent">
            <svg class="b3-menu__icon"></svg>
            <label for="" class="be-menu__label">${getI18nText("followSysAccent")}</label>
        </button>
        <button class="b3-menu__item asri-config" id="followCoverImgColor">
            <svg class="b3-menu__icon"></svg>
            <label for="" class="be-menu__label">${getI18nText("followCoverImgColor")||"follow cover img color"}</label>
        </button>
        <button class="b3-menu__item asri-config" data-type="nobg" id="asriChroma">
            ...
            <div aria-label="${
              getI18nText("asriChroma") + (asriChromaSlider?.value ?? asriConfigs[runtime.mode].chroma ?? "1")
        ...
        <div class="b3-menu__submenu">
            <div class="b3-menu__items">
                <button class="b3-menu__item" id="tfp-luminous">
                    <svg class="b3-menu__icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 22"><g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><path d="m8 10.5l6.492-6.492M13.496 16L20 9.496zm-4.91-.586L19.413 4.587M8 6a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2h-8a2 2 0 0 1-2-2z"/><path d="M16 16v2a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2v-8a2 2 0 0 1 2-2h2"/></g></svg>
                    <span class="b3-menu__label">${getI18nText("tfp-luminous")}</span>
                </button>
                <button class="b3-menu__item" id="tfp-progressive">
                    <svg class="b3-menu__icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 22"><g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><path d="m8 10.5l6.492-6.492M13.496 16L20 9.496zm-4.91-.586L19.413 4.587M8 6a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2h-8a2 2 0 0 1-2-2z"/><path d="M16 16v2a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2v-8a2 2 0 0 1 2-2h2"/></g></svg>
                    <span class="b3-menu__label">${getI18nText("tfp-progressive")}</span>
                </button>                                
                <button class="b3-menu__item" id="tfp-acrylic">
                    <svg class="b3-menu__icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 22"><g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><path d="m8 10.5l6.492-6.492M13.496 16L20 9.496zm-4.91-.586L19.413 4.587M8 6a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2h-8a2 2 0 0 1-2-2z"/><path d="M16 16v2a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2v-8a2 2 0 0 1 2-2h2"/></g></svg>
                    <span class="b3-menu__label">${getI18nText("tfp-acrylic")}</span>
                </button>                
                ...
                <button class="b3-menu__item" id="tfp-disable">
                    ...
                    <span class="b3-menu__label">${getI18nText("tfp-disable")}</span>
                </button>
            </div>
        </div>
    `;
...
export function initMenuItems() {
  ...
  followSysAccentBtn!.classList.toggle(
    ...
    !asriConfigs[runtime.mode].presetPalette && runtime.followSysAccentColor
  );
  followCoverImgColorBtn!.classList.toggle(
    "b3-menu__item--selected",
    asriConfigs[runtime.mode].followCoverImgColor && !asriConfigs[runtime.mode].presetPalette && !runtime.followSysAccentColor
  );
  pickColorBtn!.classList.toggle(
    "b3-menu__item--selected",
    !asriConfigs[runtime.mode].presetPalette && !runtime.followSysAccentColor && !asriConfigs[runtime.mode].followCoverImgColor
  );
  asriChromaSlider!.value = asriConfigs[runtime.mode].chroma || "1";
  asriChromaSlider!.parentElement!.ariaLabel = getI18nText("asriChroma") + asriConfigs[runtime.mode].chroma;

  if (asriConfigs[runtime.mode].presetPalette) {
    const curPalette = document.getElementById(`${asriConfigs[runtime.mode].presetPalette}`);
    asriChromaBtn?.classList.add("b3-menu__item--disabled");

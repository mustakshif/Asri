...
1. `当前主题配置`：显示和切换当前使用的配色方案，分为 `亮色`、`暗色` 和 `全局`。`亮色` / `暗色` 模式下，配色方案仅对程序当前的外观模式生效，即亮暗色配色方案分别设置；`全局` 模式下，配色方案同时适用于程序的亮色和暗色模式；默认值为程序当前的外观模式
2. `预置配色方案`：显示和切换适用于当前主题配置的预设配色方案列表

对以下自定义选项的调整将直接应用并保存到 `当前主题配置` 所示的配色方案中：

3. `自定义主题色`：使用自选色作为主题基准色，生成相应配色方案并直接应用
4. `跟随系统强调色`：使用系统强调色作为主题基准色，`Windows` 和 `macOS` 桌面端默认启用，其他平台此选项不可见
5. `色度滑块`：调整背景和普通文本的色彩浓度，取值范围为 `0 - 5`，`0` 表示纯灰度色，默认值为 `1`
6. `色相偏移滑块`：保持背景色不变，调整强调色和文本色（如开启）与背景色之间的色相偏移，取值范围为 `-180º - 180º`，默认值为 `0º`
7. `文本色相偏移`：切换是否启用文本颜色的色相偏移，默认开启

...
* `当前主题配置`：显示和切换当前使用的配色方案，分为 `亮色`、`暗色` 和 `全局`。`亮色` / `暗色` 模式下，配色方案仅对程序当前的外观模式生效；`全局` 模式下，配色方案同时适用于程序的亮色和暗色模式；默认值为程序当前的外观模式
* `预置配色方案`：显示和切换适用于当前主题配置的预设配色方案列表
...
* `色相偏移滑块`：保持背景色不变，调整强调色和文本色与它之间的色相偏移，取值范围为 `-180 - 180`，默认值为 `0`
